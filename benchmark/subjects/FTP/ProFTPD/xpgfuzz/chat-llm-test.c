...
//测试extract_stall_message函数
void test_extract_stall_message()
{
    char *msg = "USER unn\r\nPASS nnn\r\n";
    char *res = extract_stalled_message(msg, strlen(msg));
    printf("## Extracted message: %s\n", res);
    char *ans = format_request_message(res);
    printf("## Formatted message: %s\n", ans);
    // free(res);  //在format_request_message中已经释放了res
    ck_free(ans);  // 释放格式化后的消息内存
}

void test_format_request_message()
{
    char *msg = strdup("USER unn\r\nPASS nnn\r\n");/* Duplicate S, returning an identical malloc'd string.  */
    char *res = format_request_message(msg);
    printf("## Formatted message: %s\n", res);
    ck_free(res);  // 释放格式化后的消息内存
}


void test_chat_with_llm()
{
    char *response = chat_with_llm("What is the capital of France?", "instruct", 3, 0.5);
    if (response != NULL)
    {
        printf("## Response from LLM: %s\n", response);
        free(response);
    }
    else
    {
...
//始终测试失败
void test_construct_prompt_stall()
{
    char *protocol_name = "HTTP";
    // 示例 examples
    char *examples = "Request-1:\\nGET /index.html HTTP/1.1\\nHost: example.com\\n\\nRequest-2:\\nPOST /login HTTP/1.1\\nHost: example.com\\nContent-Length: 20\\n\\nusername=admin&password=1234\\n";

    // 示例 history
    char *history = "GET /index.html HTTP/1.1\\nHost: example.com\\n\\nHTTP/1.1 200 OK\\nContent-Type: text/html\\n\\n<html>...</html>\\nPOST /login HTTP/1.1\\nHost: example.com\\nContent-Length: 20\\n\\nusername=admin&password=1234\\nHTTP/1.1 302 Found\\nLocation: /dashboard\\n";

    char *prompt = construct_prompt_stall(protocol_name, examples, history);
    printf("## Constructed Prompt:\n%s\n", prompt);
    char * ans = chat_with_llm(prompt, "instruct", 3, 0.5);
    if (ans != NULL)
    {   
        printf("## Response from LLM:\n%s\n", ans);
        free(ans);
...
void test_construct_prompt_for_templates()
{
    char *protocol_name = "HTTP";
    char *final_msg = NULL;
    char *prompt = construct_prompt_for_templates(protocol_name, &final_msg);
    printf("## Constructed Prompt:\n%s\n", prompt);
    char * ans = chat_with_llm(prompt, "instruct", 3, 0.5);
    if (ans != NULL)
    {   
...
void  test_construct_prompt_for_protocol_message_types()
{
    char *protocol_name = "FTP";
    char *prompt = construct_prompt_for_protocol_message_types(protocol_name);
    printf("## Constructed Prompt:\n%s\n", prompt);
    char * ans = chat_with_llm(prompt, "instruct", 3, 0.5);
    if (ans != NULL)
    {   
...
{
    // test_chat_with_llm();
    // test_construct_prompt_stall();
    // test_construct_prompt_for_templates();
    // test_construct_prompt_for_protocol_message_types();
}
